...
async function handleAbout() {
  ...
  return {
    content: [{
      type: "text",
      text: `Office MCP Server\n\nProvides access to Microsoft 365 services including Outlook, Teams, OneDrive, and more through Microsoft Graph API.\n\n${formatRuntimeMetadataText(metadata)}`
    }]
  };
}

async function handleRuntimeInfo() {
  const metadata = getRuntimeMetadata(config);
  return {
    content: [{
      type: "text",
      text: formatRuntimeMetadataText(metadata)
    }]
  };
}
...
async function handleSystem(args) {
  ...
  if (!operation) {
    return {
      content: [{
        type: "text",
        text: "Missing required parameter: operation. Valid operations: about, authenticate, check_status, runtime_info"
      }]
    };
  }
  ...
  switch (operation) {
    ...
    case 'runtime_info':
      return await handleRuntimeInfo();
    default:
      return {
        content: [{
          type: "text",
          text: `Invalid operation: ${operation}. Valid operations: about, authenticate, check_status, runtime_info`
        }]
      };
  }
}
...
const authTools = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        operation: {
          type: "string",
          enum: ["about", "authenticate", "check_status", "runtime_info"],
          description: "Operation to perform: about (server info), authenticate (connect to MS Graph), check_status (verify auth), runtime_info (live runtime fingerprint)"
        },
        force: {
          type: "boolean",
