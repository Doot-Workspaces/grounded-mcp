...
describe('Teams Module - Consolidated Tools', () => {
  ...
  describe('teams_channel tool', () => {
    ...
    it('should preserve structured outbound channel messages before sending', async () => {
      callGraphAPI.mockResolvedValue({ id: 'channel-message-id' });

      await channelTool.handler({
        operation: 'create_message',
        teamId: 'team-id',
        channelId: 'channel-id',
        content: 'Update\n- Fixed locally\n- New 2-person chats will now open as one-on-one chats\n- This chat was created before the fix'
      });
      ...
      expect(callGraphAPI).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          body: {
            content: '<div>Update</div><div><br></div><ul><li>Fixed locally</li><li>New 2-person chats will now open as one-on-one chats</li><li>This chat was created before the fix</li></ul><div><br></div><div>-</div>',
            contentType: 'html'
          }
        })
      );
  ...
  describe('teams_chat tool', () => {
    ...
    it('should create a one-on-one chat for exactly two members', async () => {
      ...
      const result = await chatTool.handler({
        operation: 'create',
        members: ['owner@example.com', 'user@example.com']
      });
      ...
      expect(result.content[0].text).toContain('Chat created successfully');
      expect(result.content[0].text).toContain('One-on-One');
    });

    it('should preserve structured chat message sending', async () => {
    ...
    it('should preserve paragraph-only html chat messages', async () => {
      callGraphAPI.mockResolvedValue({ id: 'message-id' });

      const result = await chatTool.handler({
        operation: 'send_message',
        chatId: 'chat-id',
        content: '<p>Hello,</p><p>A reviewer is looking at the draft from our side.</p>'
      });
      ...
      expect(callGraphAPI).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          body: {
            content: '<div>Hello,</div><div><br></div><div>A reviewer is looking at the draft from our side.</div><div><br></div><div>-</div>',
            contentType: 'html'
          }
        })
      );
