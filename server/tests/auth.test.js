...
const { getRuntimeMetadata } = require('../utils/runtime-metadata');
...
describe('Auth Module (Consolidated System Tool)', () => {
  ...
  describe('runtime_info operation', () => {
    it('should return the live runtime fingerprint', async () => {
      const result = await systemHandler({ operation: 'runtime_info' });
      const metadata = getRuntimeMetadata(require('../config'));

      expect(result.content[0].type).toBe('text');
      expect(result.content[0].text).toContain(`Version: ${metadata.serverVersion}`);
      expect(result.content[0].text).toContain(`Entrypoint: ${metadata.entrypoint}`);
    });
  });
