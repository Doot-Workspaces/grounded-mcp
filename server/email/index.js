...
async function replyToEmail(accessToken, params) {
  try {
    ...
    const mailboxPrefix = config.getMailboxPrefix(params.mailbox);
    const createReplyEndpoint = `${mailboxPrefix}/messages/${params.emailId}/createReply`;
    await callGraphAPI(
      accessToken,
      'POST',
      createReplyEndpoint,
    ...
    const draftsResponse = await callGraphAPI(
      accessToken,
      'GET',
      `${mailboxPrefix}/mailFolders/drafts/messages`,
      null,
      {
        $top: 1,
    ...
    const replyDraft = draftsResponse?.value?.[0];

    if (!replyDraft?.id) {
      throw new Error('Reply draft was created but could not be located in Drafts.');
    }

    const updatePayload = {
      body: {
        contentType: 'HTML',
        content: mergeReplyBodyHtml(formattedReplyHtml, replyDraft.body?.content || '')
      },
      ...messageOverrides
    };
    ...
    await callGraphAPI(
      accessToken,
      'PATCH',
      `${mailboxPrefix}/messages/${replyDraft.id}`,
      updatePayload,
      null
    );
    ...
    await callGraphAPI(
      accessToken,
      'POST',
      `${mailboxPrefix}/messages/${replyDraft.id}/send`,
      null,
      null
    );
...
function mergeReplyBodyHtml(replyFragment, existingBodyContent = '') {
  const fragment = (replyFragment || '').trim();
  const existing = (existingBodyContent || '').trim();

  if (!existing) {
    return wrapHtmlBody(`<div class="mcp-reply-block">${fragment}</div>`);
  }

  const bodyMatch = existing.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
  const existingInner = (bodyMatch ? bodyMatch[1] : existing).trim();

  const mergedBody = [
    '<div class="mcp-reply-block">',
    fragment,
    '</div>',
    '<div>&nbsp;</div>',
    existingInner
  ...
  return wrapHtmlBody(mergedBody);
}
