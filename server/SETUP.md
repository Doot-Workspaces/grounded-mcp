...
If the flow is blocked pending approval, use generic wording in docs and support notes:
...
After any restart, verify the live runtime before trusting behavior:

```bash
npm run runtime:info
```

If you need to validate Outlook reply formatting or Teams paragraph spacing after restart, run:
...
Use a self-addressed mailbox and a private chat only.
...
- If Microsoft says approval is pending, ask an Entra admin to grant consent for the delegated permissions
